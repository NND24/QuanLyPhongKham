...
public class GenerateChiDinh {
    ...
    public GenerateChiDinh(String ten, String tuoi, String gioiTinh, String diaChi, String chuanDoan, String ngay, String thang, String nam) {
        this.ten = ten;
        this.tuoi = tuoi;
    ...
    public void setTenDichVu(List<String> tenDichVu) {
        this.tenDichVu = tenDichVu;
    }

    public List<String> getSoLan() {
        return soLan;
    }

    public void setSoLan(List<String> soLan) {
        this.soLan = soLan;
    }
...
//        table.addCell(new Cell().add(nestedTable).setBorder(Border.NO_BORDER));
        Paragraph centerParagraph = new Paragraph("CHỈ ĐỊNH CẬN LÂM SÀN").setBold().setTextAlignment(TextAlignment.CENTER);
        ...
        Table chiDinhTable = new Table(new float[]{4, 1});
        chiDinhTable.setWidth(UnitValue.createPercentValue(100));

        for (int i = 0; i < tenDichVu.size(); i++) {
            Paragraph tenChiDinhParagraph = new Paragraph((i + 1) + "/ ")
                    .add(new Paragraph(tenDichVu.get(i)));

            Paragraph soLuongParagraph = new Paragraph(soLan.get(i) + " ")
                    .add(new Paragraph("Lần").setBold());

            chiDinhTable.addCell(new Cell().add(tenChiDinhParagraph).setBorder(Border.NO_BORDER));
            chiDinhTable.addCell(new Cell().add(soLuongParagraph).setBorder(Border.NO_BORDER));
        }
